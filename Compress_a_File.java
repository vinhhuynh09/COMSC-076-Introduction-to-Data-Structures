 ...
 ***********************************************************************************************************************
 * Description:
 Part I: Write a program to compress a source file into a target file using the Huffman coding method. First, use
 ObjectOutputStream to output the Huffman codes into the target file, then use BitOutputStream from Assignment #2 to
 output the encoded binary contents to the target file. Pass the files from the command line using something like the
 following command:

 C:\Users\.... > java Compress_a_File sourceFile.txt compressedFile.txt

 Part 2 for this assignment is in a separate file, Decompress_a_File.java.
 **********************************************************************************************************************/
...
public class Compress_a_File {
    public static void main(String[] args) throws IOException{
        if (args.length != 2) { // args.length must be exactly 2. args[0] is the source file
            // args[1] is the target file
            System.out.println("Usage: java CompressFile sourceFile targetFile");
            // from cmd prompt, type : java className source.dat target.dat
            System.exit(1);
        }

        File sourceFile = new File(args[0]);    // args[0] is the input file (source file)
        if (!sourceFile.exists()) {
            System.out.println("File " + args[0] + " does not exist");
            System.exit(2);
        }

        DataInputStream sourceFileInput = new DataInputStream(new BufferedInputStream(new FileInputStream(sourceFile)));
        int size = sourceFileInput.available();
        byte[] b = new byte[size];
        sourceFileInput.read(b);
        ...
        for (int i = 0; i < text.length(); i++) {
            result.append(code[text.charAt(i)]);
        }

        ObjectOutputStream codeOutputStream = new ObjectOutputStream(new FileOutputStream(args[1]));
        codeOutputStream.writeObject(code);
        codeOutputStream.writeInt(result.length());
